...
3. **指标翻译（Indicator Translate）**：
   ...
   - global_note 只综合陈述各指标的数值状态
   - ✅ 正确："RSI 44.50 低于 30 日均值 7.8 个点，MACD DIF 由正转负已持续 3 日，成交量连续 5 日低于均量"
...
4. **误读风险（Risk of Misreading）**：
   ...
   - caution_note 只说明需要观察的数据点
   - ✅ 正确："观察价格是否突破 11.65 或跌破 11.30，观察成交量是否超过 20 日均量 1.5 倍"
   - ❌ 错误："需要等待放量突破确认，避免追高"
...
{{
  ...
  "indicator_translate": {{
    ...
    "global_note": "RSI 44.50 低于 30 日均值 7.8 个点，MACD DIF 由正转负已持续 3 日，成交量连续 5 日低于均量"
  }},
  "risk_of_misreading": {{
    "risk_level": "high|medium|low",
    "risk_factors": [
