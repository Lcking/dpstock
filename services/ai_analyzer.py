...
class AIAnalyzer:
    ...
    async def get_ai_analysis(self, df: pd.DataFrame, stock_code: str, stock_name: str = "", market_type: str = 'A', stream: bool = False) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            # 发送精简的 K 线数据：只保留关键列，CSV 格式减少 token 消耗
            slim_cols = ['Date', 'Close', 'Volume', 'MA5', 'MA20', 'RSI']
            slim_cols = [c for c in slim_cols if c in df.columns]
            recent_df = df.tail(30)[slim_cols].copy()
            for c in recent_df.select_dtypes(include=['float64']).columns:
                recent_df[c] = recent_df[c].round(2)
            recent_data = recent_df.to_csv(index=False)
...
**数据整理规则：**
- 只陈述客观事实和历史数据，不做预测，不暗示方向
- 描述价格与均线位置关系时用具体数值，如"价格位于MA20下方0.24元"
- 引用形态检测算法结果，标注关键拐点价格和日期
- 指标翻译增加百分位排名和分布统计
- 误读风险只列客观因素，不给操作建议
- 判断区只描述触发条件，把决策权交给用户
