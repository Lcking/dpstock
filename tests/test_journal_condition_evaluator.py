...
def test_review_record_persists_system_evaluation_from_saved_candidate_conditions(tmp_path):
    db_path = tmp_path / "journal_eval.db"
    user_id = "user_eval"
    record_id = "jr_eval_1"
...
def test_evaluate_record_returns_system_evaluation_without_marking_reviewed(tmp_path):
    db_path = tmp_path / "journal_preview.db"
...
def test_run_due_check_stores_evaluation_preview_for_expired_active_record(tmp_path):
    db_path = tmp_path / "journal_due_preview.db"
...
def test_review_record_reuses_existing_evaluation_preview_without_refetching_data(tmp_path):
    db_path = tmp_path / "journal_review_preview.db"
...
def test_review_record_persists_lesson_summary(tmp_path):
    db_path = tmp_path / "journal_review_lesson.db"
...
def test_review_record_persists_failure_reason(tmp_path):
    db_path = tmp_path / "journal_review_failure.db"
