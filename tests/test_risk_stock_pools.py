...
def test_st_omen_rows_follow_delisting_warning_rules(monkeypatch):
    ...
    yjbb_df = pd.DataFrame(
        [
            # 主板：亏损且营收 2 亿 < 3 亿 → 命中
            {"股票代码": "600300", "股票简称": "亏损小营收", "净利润-净利润": -5e7, "营业总收入-营业总收入": 2e8, "每股净资产": 1.2},
            # 主板：亏损但营收 10 亿 → 不命中
            {"股票代码": "600301", "股票简称": "亏损大营收", "净利润-净利润": -5e7, "营业总收入-营业总收入": 1e9, "每股净资产": 2.0},
            # 创业板：亏损且营收 0.8 亿 < 1 亿 → 命中
            {"股票代码": "300400", "股票简称": "创业板亏损", "净利润-净利润": -1e7, "营业总收入-营业总收入": 8e7, "每股净资产": 0.5},
            # 创业板：亏损但营收 2 亿（主板阈值以下，但创业板阈值以上）→ 不命中
            {"股票代码": "300401", "股票简称": "创业板营收过线", "净利润-净利润": -1e7, "营业总收入-营业总收入": 2e8, "每股净资产": 1.0},
            # 盈利 → 不命中
            {"股票代码": "600302", "股票简称": "盈利示例", "净利润-净利润": 5e7, "营业总收入-营业总收入": 1e8, "每股净资产": 3.0},
            # 已戴帽 → 排除
            {"股票代码": "600305", "股票简称": "*ST已帽", "净利润-净利润": -5e7, "营业总收入-营业总收入": 1e8, "每股净资产": 0.1},
            # 每股净资产为负 → 命中（即使盈利）
            {"股票代码": "600306", "股票简称": "负净资产", "净利润-净利润": 1e7, "营业总收入-营业总收入": 5e8, "每股净资产": -0.8},
            # 科创板未盈利豁免：亏损+低营收，但不进池
            {"股票代码": "688765", "股票简称": "未盈利生物", "净利润-净利润": -1e8, "营业总收入-营业总收入": 5e7, "每股净资产": 2.0},
            # 科创板 -U 标记豁免
            {"股票代码": "688806", "股票简称": "N泰诺-U", "净利润-净利润": -1e7, "营业总收入-营业总收入": 1e7, "每股净资产": 5.0},
        ]
    )
    basic_df = pd.DataFrame(
        [
            ...
            {"ts_code": "002775.SZ", "pb": float("nan")},  # NaN 不得当负净资产
        ]
    )
    ...
    monkeypatch.setattr(
        RiskStockCollector,
        "_build_name_map",
        lambda self: {"600303": "负资产示例", "002775": "NaN误伤"},
    )
    monkeypatch.setattr(
        RiskStockCollector,
        "_is_star_pre_profit_exempt",
        lambda self, ts_code, name="": ts_code.startswith("688"),
    )
    ...
    assert "600306.SH" in by_code
    assert "净资产为负" in by_code["600306.SH"]["pool_reasons"][0]
    assert "600303.SH" in by_code
    assert "净资产为负" in by_code["600303.SH"]["pool_reasons"][0]
    assert "600304.SH" not in by_code
    assert "002775.SZ" not in by_code  # PB=NaN 不得入池
    assert "688765.SH" not in by_code  # 科创板未盈利豁免
    assert "688806.SH" not in by_code
